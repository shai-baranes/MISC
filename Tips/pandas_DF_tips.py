...
basic funcs to get acquaintet with your df:
	...
	df.columns # you get a list of the DF columns
	df.describe() # for statistics - works only on the mumeric fields!
	df['Story Points (Hours)'].describe() # works on the specified column
	df.tail() # 5 last rows (you can input any argument value - int)
	df.head() # 5 first rows (you can input any argument value - int)
	df.sample(10)  # gets randomed lines
	df.shape # matrix size (rows and columns)
	df.to_csv('file_name.csv') # getting df or manipulated df into a csv file
	df['Reporter'].unique() # list of all unique reporters (unique values under this column)
...
conventional Plots (_with annotations);
